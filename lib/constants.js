...
exports.LOGERROR = (e, logBinary, stacktrace=true) => {
    if (!logBinary) console.error(_getAsString('ERROR', getErrorMessage(e, stacktrace), COLOR.fgRed));
    else process.stderr.write(_getAsBuffer('ERROR', getErrorMessage(e, stacktrace), COLOR.fgRed), "binary");
}
...
exports.LOGFAILURE = _ => exports.LOGERROR("Build failed.", false, false);
...
function getErrorMessage(e, stacktrace) {
    const stack = new Error("Stack trace").stack;

    const type = typeof e; const keys = Object.keys(e); let errorMessage = e instanceof Error ? e.message : e.toString(); 
    if (e && (!(e instanceof Error)) && ((type === 'function') || (type === 'object')) && keys.length) errorMessage = JSON.stringify(e);

    const finalMessage = `${errorMessage}${stacktrace?`\n[ERROR] ${stack}`:""}`; return finalMessage;
}
