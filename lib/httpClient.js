...
function _addHeaders(headers, body) {
    if (body) headers["content-type"] = _getObjectKeyValueCaseInsensitive(headers, "content-type") || "application/x-www-form-urlencoded";
    if (body) headers["content-length"] = Buffer.byteLength(body, "utf8");
    headers["accept"] = _getObjectKeyValueCaseInsensitive(headers, "accept") || "*/*";
}
...
function _doCall(reqStr, options, secure, sslObj) {
    ...
    return new Promise(async (resolve, reject) => {
        ...
        if (secure && (!sslObj?._org_monkshu_httpclient_forceHTTP1)) { // for http2 case
            CONSTANTS.LOGDEBUG(`httpClient connecting to URL ${options.host}:${options.port}/${options.path} via HTTP2.`);
            ...
            req.on("response", headers => {
                const encoding = _getObjectKeyValueCaseInsensitive(headers, "content-encoding") || "identity";
            ...
            CONSTANTS.LOGDEBUG(`httpClient connecting to URL ${options.host}:${options.port}/${options.path} via HTTP1.`);

            if (sslObj & typeof sslObj == "object") try{await _addSecureOptions(options, sslObj)} catch (err) {reject(err); return;};
            const req = caller.request(options, res => {
                const encoding = _getObjectKeyValueCaseInsensitive(res.headers, "content-encoding") || "identity";
...
function _getObjectKeyValueCaseInsensitive(obj, key) {
